...
namespace KTG.Areas.Setup.ViewModels
{
	public class SetupViewModel
	{
		...
		public SelectList HechsherList()
		{
			Db db = new Db();
			List<SelectListItem> lst = new List<SelectListItem>();
			foreach (var h in db.Hechshers)
			{
				var a = new SelectListItem() { Text = h.Name, Value = h.id.ToString() };
				lst.Add(a);
			};
			return new SelectList(lst, "Value", "Text");
		}

		public string HechsherName(string id)
		{
			Db db = new Db();
			int hid = Int32.Parse(id);
			return db.Hechshers.Find(hid).Name;
		}
	}
}